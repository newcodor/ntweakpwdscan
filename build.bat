...
@set GOOS=windows&&set GOARCH=386&& go1.20.14 build  -trimpath -ldflags "-w -s"   -o %build_dir%/%outfilename%  main.go
...
@set GOOS=linux&&set GOARCH=386&& go1.20.14 build -trimpath  -ldflags "-w -s"  -o %build_dir%/%outfilename%  main.go

@set outfilename=%output%_linux_amd64_%version%
@echo build linux/amd64 …… %build_dir%/%outfilename%
@set GOOS=linux&&set GOARCH=amd64&& go1.20.14 build -trimpath  -ldflags "-w -s" -o %build_dir%/%outfilename%  main.go
